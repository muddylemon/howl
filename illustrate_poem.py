...
def create_video(audio_file, output_file, images_folder, lines_file, fps):
...
def main():
    ...
    # Generate the output file name from the images_folder path
    folder_name = os.path.basename(os.path.normpath(args.images_folder))
    output_file = os.path.join("videos", f"{folder_name}.mp4")

    create_video(args.audio_file, output_file, args.images_folder, args.lines_file, args.fps)
